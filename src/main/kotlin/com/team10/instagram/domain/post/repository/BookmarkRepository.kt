...
interface BookmarkRepository : CrudRepository<Bookmark, Long> {
    ...
    fun existsByPostIdAndUserId(@Param("postId") postId: Long, @Param("userId") userId: Long): Boolean

    @Query("SELECT * FROM bookmark WHERE post_id = :postId AND user_id = :userId")
    fun findByPostIdAndUserId(@Param("postId") postId: Long, @Param("userId") userId: Long): Bookmark?

    // 내가 북마크한 게시글의 ID 목록 가져오기 (이후 PostRepository에서 조회)
